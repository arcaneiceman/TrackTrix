...
namespace Traktrix.Filters
{
    class LowPassFilter :Filter
    {
        internal static byte[] FilterTheBuffer(byte[] input, int Bf,int cutoff)
        {
            ...
            return DoublesToBytes(LowPassF(AudioinDouble,cutoff),Bf);
        }


        public static double[] LowPassF(double[] audioData, int cutoff)
        {
            ...
            var LowPass = BiQuadFilter.LowPassFilter(44100, cutoff, 4f);
            float temp;
            for (int i = 0; i < audioData.Length; i++)
            {
                audioData[i] = audioData[i] * 0.5d;
                temp = DoubletoFloat(audioData[i]);
                temp = LowPass.Transform(temp);
